...
///Macro to generate the scaling to reweight the Signal PU distribution to match the SM PU distribution (which is subsequently reweighted to match Data
...
void PURWHistMaker() {
    ...
    TFile * outputFile = new TFile("S7toS10RW.root", "RECREATE");
    TH1F * outHist = new TH1F("S7toS10RWHist", ";nVtx;Weight", 60, 0.5, 60.5); outHist->Sumw2();
    Double_t currWeight;
    for (int i = 0; i < 60; ++i) {
        currWeight = Summer2012_S10[i]/Summer2012_S7[i];
	if (i > 44) currWeight = Summer2012_S10[44]/Summer2012_S7[44];
        outHist->SetBinContent(i + 1, currWeight);
    }
    outputFile->cd();
    outputFile->Write();
    outputFile->Close();
