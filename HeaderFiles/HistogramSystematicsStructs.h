...
typedef struct ValError {
    ...
    void PrintOut(TString inName, bool isStat = true, TString systName = "", bool printRelErr= false, bool printSymSyst = false) {
        ...
        if (isStat) {
            if (printRelErr) {
                sprintf(buf,"CentVal: %0.2f, StatErr: %0.2f", centVal, 1 + (upError/centVal));
                cout << "Name: " << inName << ", " << buf << endl;   
            }
            else {
                sprintf(buf,"CentVal: %0.2f, StatErr: %0.2f", centVal, upError);
                cout << "Name: " << inName << ", " << buf << endl;                
            }
        }
        else {
            if (printSymSyst) {
                if (printRelErr) {
                    sprintf(buf,"CentVal: %0.2f, AverageErr: %0.2f", centVal, 1 + ((0.5/centVal) * (upError + downError)));
                    cout << "Name: " << inName << ", Syst: " << systName << ", " << buf << endl;                
                }
                else {
                    sprintf(buf,"CentVal: %0.2f, AverageErr: %0.2f", centVal, 0.5 * (upError + downError));
                    cout << "Name: " << inName << ", Syst: " << systName << ", " << buf << endl;
                }
            }
            else {
                if (printRelErr) {
                    sprintf(buf,"CentVal: %0.2f, UpErr: %0.2f, DownErr: %0.2f", centVal, 1 + (upError/centVal), 1 + (downError/centVal));
                    cout << "Name: " << inName << ", Syst: " << systName << ", " << buf << endl;                
                }
                else {
                    sprintf(buf,"CentVal: %0.2f, UpErr: %0.2f, DownErr: %0.2f", centVal, upError, downError);
                    cout << "Name: " << inName << ", Syst: " << systName << ", " << buf << endl;
                }
            }
        }
