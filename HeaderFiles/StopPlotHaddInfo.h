...
vector<double> * WeightVec(float L_data, vector<double> * baseWeightVec, unsigned int whichFileList, vector<int> * numParFilesVec, bool doExclusiveSample) {
    vector<double> * outVec = new vector<double>;    
    int numParFiles;
    ...
    switch (whichFileList) {
        case 0:
            ...
            if (doExclusiveSample) {
                ...
                outVec->push_back(numParFiles * L_data * xsecTTbarSemiLeptonic / baseWeightVec->at(1));
                numParFiles = numParFilesVec->at(2);
                outVec->push_back(numParFiles * L_data * xsecTTbarHadronic / baseWeightVec->at(2));
            }
            else {
...
vector<TFile *> * OutFileVec(TString TTBarSystString, TString mainString, vector<bool> * boolVec, bool doExclusiveSample) {
    vector<TFile *> * outVec = new vector<TFile *>;
    TFile * TargetTTBarComp, * TargetSingTop, * TargetZDY, * TargetWLNu, * TargetWW, * TargetWZ, * TargetZZ;
    ...
    TString suffixString = TString("Haddplots.root");
    if (boolVec->at(0)) {
        TargetTTBarComp = TFile::Open(TString("TTBarComp") + TTBarSystString + exclusiveString + mainString + suffixString, "RECREATE");
        outVec->push_back(TargetTTBarComp);
    }
    if (boolVec->at(1)) {
        TargetZDY = TFile::Open(TString("ZDY") + exclusiveString + mainString + suffixString, "RECREATE");
        outVec->push_back(TargetZDY);
    }
    if (boolVec->at(2)) {
        TargetSingTop = TFile::Open(TString("SingleTop") + mainString + suffixString, "RECREATE");
        outVec->push_back(TargetSingTop);
    }
    if (boolVec->at(3)) {
        TargetWW = TFile::Open(TString("WW") + mainString + suffixString, "RECREATE");   
        outVec->push_back(TargetWW);
        TargetWZ = TFile::Open(TString("WZ") + mainString + suffixString, "RECREATE");   
        outVec->push_back(TargetWZ);
        TargetZZ = TFile::Open(TString("ZZ") + mainString + suffixString, "RECREATE"); 
        outVec->push_back(TargetZZ);
    }
    if (boolVec->at(6)) {
        TargetWLNu = TFile::Open(TString("WToLNu") + mainString + suffixString, "RECREATE");
        outVec->push_back(TargetWLNu);
    }
    if (boolVec->at(7)) {
        TargetWG = TFile::Open(TString("WG") + mainString + suffixString, "RECREATE");   
        outVec->push_back(TargetWG);
        TargetZG = TFile::Open(TString("ZG") + mainString + suffixString, "RECREATE");   
        outVec->push_back(TargetZG);
    }        
    if (boolVec->at(9)) {
        TargetHiggsWW = TFile::Open(TString("HiggsWW") + mainString + suffixString, "RECREATE");                
        outVec->push_back(TargetHiggsWW);
        TargetHiggsVBF = TFile::Open(TString("HiggsVBF") + mainString + suffixString, "RECREATE"); 
        outVec->push_back(TargetHiggsVBF);
        TargetHiggsZZ4L = TFile::Open(TString("HiggsZZ4L") + mainString + suffixString, "RECREATE"); 
        outVec->push_back(TargetHiggsZZ4L);
    } 
    if (boolVec->at(12)) {
        TargetTripleVecBoson = TFile::Open(TString("TripVecBoson") + mainString + suffixString, "RECREATE");                
        outVec->push_back(TargetTripleVecBoson);
        TargetTTBarVecBoson = TFile::Open(TString("TTBarVecBoson") + mainString + suffixString, "RECREATE");                
        outVec->push_back(TargetTTBarVecBoson);
    }    
    return outVec;
}

vector<TList *> * FileListVec(TString TTBarSystString, TString mainString, vector<bool> * boolVec, bool doExclusiveSample) {
    vector<TList *> * outVec = new vector<TList *>;
    TList * FileListTTBarComp, * FileListSingTop, * FileListZDY, * FileListWLNu, * FileListWW, * FileListWZ, * FileListZZ, * FileListQCDMu, * FileListQCDEM, * FileListQCDBCEM, * FileListZG, * FileListWG, * FileListHiggsWW, * FileListHiggsVBF, * FileListHiggsZZ4L; 
    TList * FileListTripVecBoson, * FileListTTBarVecBoson;   
    TString suffixString = "_Output.root";
    if (boolVec->at(0)) {
        FileListTTBarComp = new TList();
        if (doExclusiveSample) {
            FileListTTBarComp->Add(TFile::Open(TString("TTJetsFullLeptMGtauola") + mainString + suffixString));
            FileListTTBarComp->Add(TFile::Open(TString("TTJetsFullHadrMG") + mainString + suffixString));
            FileListTTBarComp->Add(TFile::Open(TString("TTJetsSemiLeptMG") + mainString + suffixString));
        }
        else {
            FileListTTBarComp->Add(TFile::Open(TString("TTbar") + TTBarSystString + mainString + suffixString));
        }
        outVec->push_back(FileListTTBarComp);
    }
    if (boolVec->at(1)) {
    ...
    if (boolVec->at(2)) {
        ...
        FileListSingTop->Add(TFile::Open(TString("TWDilep") + mainString + suffixString));
        FileListSingTop->Add(TFile::Open(TString("TbarWDilep") + mainString + suffixString));
        FileListSingTop->Add(TFile::Open(TString("SingleTop_Tchannel") + mainString + suffixString));
        FileListSingTop->Add(TFile::Open(TString("SingleTopBar_Tchannel") + mainString + suffixString));
        FileListSingTop->Add(TFile::Open(TString("SingleTop_Schannel") + mainString + suffixString));
        FileListSingTop->Add(TFile::Open(TString("SingleTopBar_Schannel") + mainString + suffixString));                
        outVec->push_back(FileListSingTop);
    }
    if (boolVec->at(3)) {
        FileListWW = new TList();
        FileListWW->Add(TFile::Open(TString("WWTo2L2Nu_Madgraph") + mainString + suffixString));
        FileListWW->Add(TFile::Open(TString("ggWWto2L") + mainString + suffixString));
        outVec->push_back(FileListWW);
        FileListWZ = new TList();
        FileListWZ->Add(TFile::Open(TString("WZ") + mainString + suffixString));
        outVec->push_back(FileListWZ);
        FileListZZ = new TList();
        FileListZZ->Add(TFile::Open(TString("ZZ") + mainString + suffixString));
        outVec->push_back(FileListZZ);
    }
    if (boolVec->at(6)) {
        FileListWLNu = new TList();
        FileListWLNu->Add(TFile::Open(TString("WJets_Madgraph") + mainString + suffixString));
        outVec->push_back(FileListWLNu);
    }
    if (boolVec->at(7)) {
        FileListWG = new TList();
        FileListWG->Add(TFile::Open(TString("WgammaToLNuG") + mainString + suffixString));                
        FileListWG->Add(TFile::Open(TString("WGstarToElNuMad") + mainString + suffixString));
        FileListWG->Add(TFile::Open(TString("WGstarToMuNuMad") + mainString + suffixString));
        FileListWG->Add(TFile::Open(TString("WGstarToTauNuMad") + mainString + suffixString));            
        outVec->push_back(FileListWG);
        FileListZG = new TList();
        FileListZG->Add(TFile::Open(TString("ZgammaToLLG") + mainString + suffixString));
        outVec->push_back(FileListZG);
    }
    if (boolVec->at(9)) {
        FileListHiggsWW = new TList();
        FileListHiggsWW->Add(TFile::Open(TString("HWW125") + mainString + suffixString));
        outVec->push_back(FileListHiggsWW);
        FileListHiggsVBF = new TList();
        FileListHiggsVBF->Add(TFile::Open(TString("VBF125") + mainString + suffixString));
        outVec->push_back(FileListHiggsVBF);
        FileListHiggsZZ4L = new TList();
        FileListHiggsZZ4L->Add(TFile::Open(TString("HZZ4L") + mainString + suffixString));
        outVec->push_back(FileListHiggsZZ4L);
    }
    if (boolVec->at(12)) {
        FileListTripVecBoson = new TList();
        FileListTripVecBoson->Add(TFile::Open(TString("WWGJets") + mainString + suffixString));
        FileListTripVecBoson->Add(TFile::Open(TString("WWWJets") + mainString + suffixString));
        FileListTripVecBoson->Add(TFile::Open(TString("WWZJets") + mainString + suffixString));
        FileListTripVecBoson->Add(TFile::Open(TString("WZZJets") + mainString + suffixString));
        FileListTripVecBoson->Add(TFile::Open(TString("ZZZJets") + mainString + suffixString));
        outVec->push_back(FileListTripVecBoson); 
        FileListTTBarVecBoson = new TList();
        FileListTTBarVecBoson->Add(TFile::Open(TString("TTGJets") + mainString + suffixString));
        FileListTTBarVecBoson->Add(TFile::Open(TString("TTZJets") + mainString + suffixString));
        FileListTTBarVecBoson->Add(TFile::Open(TString("TTWJets") + mainString + suffixString));
        FileListTTBarVecBoson->Add(TFile::Open(TString("TTWWJets") + mainString + suffixString));
