...
void SetCanvases(vector<TCanvas *> * inVecCanvas, TString baseCanvName, bool dualPad, bool doLog) {
    ...
    for (unsigned int iCanv = 0; iCanv < inVecCanvas->size(); ++iCanv) {
        ...
        inVecCanvas->at(iCanv)->SetLeftMargin(0.13);
        inVecCanvas->at(iCanv)->SetRightMargin(0.03);
        inVecCanvas->at(iCanv)->SetTopMargin(0.105);
        inVecCanvas->at(iCanv)->SetBottomMargin(0.13);
        if (dualPad) {
...
void SetCanvases(vector<TCanvas *> * inVecCanvas, vector<TString> * inVecCanvasNames, bool dualPad, bool doLog) {
    ...
    for (unsigned int iCanv = 0; iCanv < inVecCanvas->size(); ++iCanv) {
        if (dualPad) {
            ...
            inVecCanvas->at(iCanv) = new TCanvas(inVecCanvasNames->at(iCanv), inVecCanvasNames->at(iCanv), singCanvWidth, singCanvHeight);
            inVecCanvas->at(iCanv)->SetLogy(doLog);
            inVecCanvas->at(iCanv)->SetGridy(1);
            inVecCanvas->at(iCanv)->SetLogx(doLog);
            inVecCanvas->at(iCanv)->SetGridx(1);
...
void FinalDrawGraph(TCanvas * inputCanvas, vector<TGraph *> * inputGraphVec, vector<TString> * inVecLegEntries, bool isEff, TString baseDir = "") {
    inputCanvas->cd(1);
    ...
    */
    ...
    inputCanvas->SaveAs(baseDir + inputCanvas->GetName() + TString(".pdf"));
    inputCanvas->SaveAs(baseDir + inputCanvas->GetName() + TString(".C"));
}
...
void InitialDrawGraphsPunzi(TCanvas * inputCanvas, vector<TGraph *> * inputGraphVec, vector<TGraph *> * inputRatioGraphVec) {
    ...
    yAxis->SetRangeUser(absMin, absMax * 1.3);
    inputCanvas->Update();
...
TString SetFileString_MT2lblb_2(int whichSig) {
    const int numSigs = 5;
    TString strSigs[numSigs] = {"T2tt", "T2bw0p25", "T2bw0p50", "T2bw0p75", "T2tb"};
    return strSigs[whichSig];
}
