...
TH2F* interpolate(const TH2F* hist, std::string firstInterpolationDirection)
{
    TH2F *histCopy = (TH2F*)hist->Clone(hist->GetName() + TString("_Interpolate"));
    ...
    TH2F *hist_step1 = (TH2F*)histCopy->Clone(histCopy->GetName() + TString("_Step1"));
    hist_step1->Reset();
    TH2F *hist_step2 = (TH2F*)histCopy->Clone(hist_step1->GetName() + TString("_Step2"));
    hist_step2->Reset();
